            ...
            if (!result_1 || result_1.length === 0) {
              ...
              .then(result_2 => {
                ...
                if (!result_2 || result_2.length === 0) {
                  ...
                  new Result(jsondata, '查询数据成功').success(res)
                }
              })
            }
          })
    ...
    .then(task => {
      if (task) {
        ...
        .then(data => {
          ...
          if (!data || data.length === 0) {
            new Result(null, '添加数据失败').fail(res)
          } else {
            new Result(null, '添加数据成功').success(res)
          }
        })
      }
    })
  ...
  if (!err.isEmpty()) {
    ...
    .then(task => {
      if (task) {
        ...
        new Result(null, '参数错误或数据不存在').fail(res)
      }
    })
    ...
    .then(task => {
      if (task) {
        ...
        .then(data => {
          ...
          if (!data || data.length === 0) {
            new Result(null, '操作数据失败').fail(res)
          } else {
            new Result(null, '操作数据成功').success(res)
          }
        })
      } else {
        new Result(null, '参数错误或数据不存在').fail(res)
    ...
    .then(task => {
      if (task) {
        ...
        .then(data => {
          ...
          if (!data || data.length === 0) {
            new Result(null, '删除数据失败').fail(res)
          } else {
            new Result(null, '删除数据成功').success(res)
          }
        })
      } else {
        new Result(null, '数据不存在').fail(res)
      }
    })
